...
def index():
    ...
    if 'mix_kw' in req:
        rule = or_(Good.name.ilike("%{0}%".format(req['mix_kw'])),Good.mobile.like("%{0}%".format(req['mix_kw'])))
...
def set():
    ...
    if name is None or len(name) < 1:
        resp['code'] = -1
        resp['msg'] = '请输入符合规范的name'
        return jsonify(resp)

    if not price or len(price) < 1:
        resp['code'] = -1
        resp['msg'] = '请输入符合规范的price'
        return jsonify(resp)
    ...
    if summary is None or len(summary) < 3:
        resp['code'] = -1
        resp['msg'] = '请输入商品描述，不少于十个字符'
        return jsonify(resp)
    
    if stock < 1:
        resp['code'] = -1
        resp['msg'] = '请输入符合规范的库存数量'
        return jsonify(resp)
